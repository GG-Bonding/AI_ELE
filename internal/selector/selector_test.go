...
func TestSelectorIgnoreLowScore(t *testing.T) {
	t.Parallel()
	sel := selector.New(selector.DefaultConfig())
	got := sel.Select("deploy kubernetes cluster", []retrieval.RankedExperience{
		ranked("e1", "Resolve Jira project key before create_issue.", retrieval.ScoreBreakdown{
			Similarity: 0.2, Utility: 0.8, Confidence: 0.9, Freshness: 1, ScopeMatch: 0.9, FinalScore: 0.02,
		}),
	})
...
func TestSelectorCompressEpisodeSpecific(t *testing.T) {
	t.Parallel()
	sel := selector.New(selector.DefaultConfig())
	...
	if got[0].Decision != selector.DecisionCompress {
		t.Fatalf("decision=%s reason=%s", got[0].Decision, got[0].Reason)
	}
	if strings.Contains(got[0].Content, "ep_abc123456") {
		t.Fatalf("compressed content still has episode id: %s", got[0].Content)
	}
}

func TestSelectorChineseTaskNotBlockedByLexicalGate(t *testing.T) {
	t.Parallel()
	sel := selector.New(selector.DefaultConfig())
	got := sel.Select("创建支付项目 Jira 问题", []retrieval.RankedExperience{
		ranked("e1", "Resolve project key before create_issue.", retrieval.ScoreBreakdown{
			Similarity: 0.8, Utility: 0.8, Confidence: 0.9, Freshness: 1, ScopeMatch: 1, FinalScore: 0.5,
		}),
	})
	if got[0].Decision != selector.DecisionKeep {
		t.Fatalf("chinese task should KEEP via embedding score, got %s (%s)", got[0].Decision, got[0].Reason)
	}
}
